...
type YCKPreemptibleKillerState struct {
...
var (
	...
	nodeTotals = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "yck_preemptible_killer_node_totals",
...
func getCurrentNodeState(node v1.Node) (state YCKPreemptibleKillerState) {
	var ok bool

	state.ExpiryDatetime, ok = node.ObjectMeta.Annotations[annotationYCKPreemptibleKillerState]
...
func getDesiredNodeState(now time.Time, ctx context.Context, kubernetesClient KubernetesClient, node v1.Node) (state YCKPreemptibleKillerState, err error) {

	twelveHours := 12 * time.Hour
	// We actually use 22 hours here before it gets chance to be removed by the Yandex Cloud itself
	twentyFourHours := 22 * time.Hour
